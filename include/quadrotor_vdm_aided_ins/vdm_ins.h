...
#include <quadrotor_vdm_aided_ins/MotorRPM.h>
#include <quadrotor_vdm_aided_ins/ParameterEstimates.h>
...
struct Consts
{
  ...
  // previously dynamic states of Wuest, but for now set as parameters
  // inertia
  double m;
  mat3 I;
  vec3 r_BM;
  ...
  vec4 k_f, k_m;
  // rotor arm length
  Eigen::Matrix<flt,3,NUM_ROTORS> r_BR;

  // matrices for wrench calculation from rotor rpms
  Eigen::Matrix<flt,3,4> P_f,P_M;
  int num_rotors;

  // x-y projection for drag force ratio matrix
  Eigen::Matrix<flt,3,3> P_xy;
...
public:
  ...
  void rpm_callback (const quadrotor_vdm_aided_ins::MotorRPM::ConstPtr &msg);
...
private:
  ...
  void measUpdateAltitudeUKF(StateWithCov &state, MeasPose &meas);
  ...
  float measurementModelAltitude(State& X);
  ...
  void rpmMsg2input(Input &returnVar, const quadrotor_vdm_aided_ins::MotorRPM::ConstPtr &msg);
